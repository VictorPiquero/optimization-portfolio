...
var asig{CIUDADES,CIUDADES} binary;
var visited{CIUDADES}>=0 integer;

minimize total_cost:
         sum{i in CIUDADES, j in CIUDADES}(asig[i,j]*dist[i,j]);

subject to lim_enter_ciu{j in CIUDADES}:
        sum{i in CIUDADES}(asig[i,j]) =1;

subject to lim_leave_ciu{i in CIUDADES}:
        sum{j in CIUDADES}(asig[i,j]) =1;
        
subject to subtour{i in CIUDADES, j in CIUDADES:i>2 and i!=j and j>2}:
        visited[i]+1<=visited[j]+n*(1-asig[i,j]);
